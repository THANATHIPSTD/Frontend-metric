...
type GameWithPosition = Game & { newReleasePosition?: number | null }
...
const selectedGames = computed(() => {
  return allGames.value
    .filter((g) => g.newReleasePosition !== null && g.newReleasePosition !== undefined)
    .sort((a, b) => (a.newReleasePosition || 0) - (b.newReleasePosition || 0))
})
...
async function fetchGames() {
  ...
  try {
    ...
    const newRelease = await backofficeApiService.fetchNewReleaseGamesForManagement()
    const newReleaseMap = new Map(newRelease.map((g, index: number) => [g.id, index + 1]))

    const mergedGames: GameWithPosition[] = all.map((game) => {
      const position = newReleaseMap.get(game.id)
      return {
        ...game,
        newReleasePosition: position !== undefined ? position : null,
      } as GameWithPosition
    })
...
function move(game: GameWithPosition, direction: 'up' | 'down') {
  ...
  const tempPos = game.newReleasePosition!
  game.newReleasePosition = targetGame.newReleasePosition!
  targetGame.newReleasePosition = tempPos
...
function handleAdd(game: GameWithPosition) {
  const nextPosition =
    (selectedGames.value[selectedGames.value.length - 1]?.newReleasePosition || 0) + 1
  game.newReleasePosition = nextPosition
  reindexSelectedGames()
}

function handleRemove(game: GameWithPosition) {
  game.newReleasePosition = null
  nextTick(() => {
    reindexSelectedGames()
  })
}
...
async function handleSave() {
  ...
  try {
    ...
    idsToRemove.forEach((id) => {
      promises.push(backofficeApiService.setGamePosition(id, 'new-release', null))
    })

    selectedGames.value.forEach((game, index) => {
      promises.push(backofficeApiService.setGamePosition(game.id, 'new-release', index + 1))
    })

    await Promise.all(promises)
    alert('New Release order saved successfully!')
    await fetchGames()
  } catch (err: unknown) {
    errorMsg.value = err instanceof Error ? err.message : 'Failed to save New Release order.'
  } finally {
...
<template>
  <div class="p-4 sm:p-8">
    ...
    >
      <h2 class="text-2xl sm:text-3xl font-bold">New Release Management (Games for you)</h2>
      <button
        @click="handleSave"
        :disabled="submitting"
      ...
      >
        ...
        <span v-else>Save New Release Order</span>
      </button>
    </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div class="space-y-4">
        <h3 class="text-xl font-semibold border-zinc-300 pb-2">
          Selected New Release Games ({{ selectedGames.length }})
        </h3>
        ...
        <div class="max-h-[70vh] overflow-y-auto border border-zinc-300 rounded-lg shadow-inner">
          <ul v-if="selectedGames.length > 0" class="divide-y divide-zinc-200 rounded-4xl">
            <li
              ...
              :key="game.id"
              class="flex justify-between items-center p-3 bg-zinc-100 shadow-inner"
            >
          ...
          <div v-else class="text-center p-6 text-zinc-500">
            <div v-if="isLoading" class="space-y-3">
              <div
                ...
                class="flex items-center gap-3 p-3 border-b border-zinc-200 animate-pulse"
              >
                <div class="w-6 h-6 bg-gray-200 rounded shrink-0"></div>
                <div class="w-10 h-10 bg-gray-200 rounded-md shrink-0"></div>
                <div class="flex-1 space-y-2">
                  <div class="h-4 bg-gray-200 rounded w-3/4"></div>
                </div>
            ...
            <div v-else class="text-center p-6 text-zinc-500">No games selected.</div>
          </div>
        </div>
      </div>
    </div>
